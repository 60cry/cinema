...
export async function generateMetadata(
  ...
  if (!id) {
    // Changed title for anime context
  ...
  try {
    ...
    // Determine if it's an anime for type parameter and URL construction
    const isStrictlyAnime = tvShow.genres?.some(g => g.id === 16); // Remove the original_language check
    const mediaTypeForPath = isStrictlyAnime ? 'anime' : 'tv'; // Use this for URL paths
    // pageUrl should reflect the actual path of this file, which is /anime/
    ...
    const commentsForSchema = await fetchSsrCommentsDataForSchema(tvShow.id, mediaTypeForPath); // Use mediaTypeForPath
    ...
    // Ensure titlePrefix is 'انمي' if it's an anime page
    ...
    const metadata: Metadata = {
      ...
      alternates: {
        canonical: pageUrl, // Ensure this uses the /anime/ path
      },
      openGraph: {
        title: `${titlePrefix} ${tvShow.name}${year ? ` (${year})` : ''} | ${siteName}`, // Use titlePrefix
        description: `شاهد وحمل جميع حلقات ${titlePrefix} ${tvShow.name} مترجم اون لاين. ${tvShow.overview?.substring(0, 100) || ''}`,
        url: pageUrl, // Ensure this uses the /anime/ path
        siteName: siteName,
        images: [
          {
    ...
    const jsonLd = {
      ...
      '@type': 'TVSeries', // Anime is a type of TVSeries
      ...
      containsSeason: tvShow.seasons?.map(season => ({
        ...
        url: `${pageUrl}/season/${season.season_number}`, // URL to the season page, should use /anime/ base
      })) || [],
      potentialAction: {
        '@type': 'WatchAction',
        target: {
    ...
    // BreadcrumbList Schema for Anime Page
    const breadcrumbSchema = {
      ...
      itemListElement: [
        ...
        },
        {
          '@type': 'ListItem',
          position: 2,
          name: 'انمي', // Anime specific category
          item: `${siteUrl}/anime`,
        },
        {
          '@type': 'ListItem',
          position: 3,
          name: tvShow.name,
          item: pageUrl, // Already correctly set to /anime/[slug]
        },
      ],
    };
    ...
    metadata.other['json-ld'] = JSON.stringify({
        '@context': 'https://schema.org',
        '@graph': [
            jsonLd,       // TVSeries schema adapted for anime
            faqSchema,    // FAQ schema
            reviewsSchema.length > 0 ? reviewsSchema : undefined, // Conditionally add reviews if they exist
            breadcrumbSchema // Added BreadcrumbList schema
        ].filter(Boolean) // Filter out undefined reviewsSchema if empty
    });
    ...
    // Ensure slugForError is defined if paramsPromise was resolved.
...
export default async function AnimeDetailPage(
    ...
    try {
        ...
        if (!preliminaryTvShowDetailsForType) {
            notFound(); // If show details can't be fetched, not found.
        }
        // Ensure it's consistently treated as anime for this page
        ...
        // Ensure breadcrumbs are for anime
        const breadcrumbCategoryLabel = "انمي";
        const breadcrumbCategoryLink = "/anime";
        const breadcrumbMediaPrefix = "مشاهدة انمي";
        ...
        const breadcrumbItems: BreadcrumbItem[] = [
            ...
            { label: breadcrumbCategoryLabel, href: breadcrumbCategoryLink },
            {
                ...
                href: pageUrl, // Use the /anime/ path
                isCurrent: true,
            },
        ];
        ...
        return (
            <Suspense fallback={<div className="h-[60vh] min-h-[400px] w-full flex items-center justify-center bg-gray-200 dark:bg-gray-700 animate-pulse" />}>
                ...
                <MediaDetail
                    item={tvShow}
                    type={"anime"} // Hardcode type as anime for this page
                    initialComments={ssrComments} 
                    recommendedMediaContent={
                        recommendations && recommendations.length > 0 ? (
                            <RecommendedMedia items={recommendations} title="قد يعجبك أيضاً" itemType={"anime"} /> // Hardcode itemType
                        ) : null
                    }
                />
            </Suspense>
